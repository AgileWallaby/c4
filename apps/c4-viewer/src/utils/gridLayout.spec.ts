...
function makeNode(id: string, x: number, y: number, type = 'softwareSystemNode', parentId?: string): Node {
    return {
        id,
        type,
        position: { x, y },
...
describe('isGridNode', () => {
    ...
    it('returns true for child nodes (have parentId)', () => {
        expect(isGridNode(makeNode('child', 0, 0, 'containerNode', 'g1'))).toBe(true)
    })
...
describe('applyInitialGridLayout', () => {
    it('places top-level nodes left-to-right, top-to-bottom', () => {
        ...
        expect(result.find((n) => n.id === 'a')!.position).toEqual(cellCenter(0, 0))
        expect(result.find((n) => n.id === 'b')!.position).toEqual(cellCenter(0, 1))
        expect(result.find((n) => n.id === 'c')!.position).toEqual(cellCenter(1, 0))
        expect(result.find((n) => n.id === 'd')!.position).toEqual(cellCenter(1, 1))
    })
...
describe('buildOccupancyMap', () => {
    ...
    it('handles multiple nodes in different cells', () => {
        const nodes = [makeNode('a', cellCenter(0, 0).x, cellCenter(0, 0).y), makeNode('b', cellCenter(1, 2).x, cellCenter(1, 2).y)]
        const map = buildOccupancyMap(nodes, 3, 3)
        expect(map.get('0,0')).toBe('a')
