...
export function cellCenter(row: number, col: number, cellWidth = CELL_WIDTH, cellHeight = CELL_HEIGHT): { x: number; y: number } {
    return {
 ...
 */
export function recomputeGroupSizes(nodes: Node[], cellWidth = CELL_WIDTH, cellHeight = CELL_HEIGHT): Node[] {
    void cellWidth
    void cellHeight
    ...
    for (const n of nodes) {
        if (!n.parentId) {
            ...
            const parent = nodeMap.get(n.parentId)
            const pPos = parent ? (absPos.get(n.parentId) ?? parent.position) : { x: 0, y: 0 }
            absPos.set(n.id, {
    ...
    for (const groupNode of nodes.filter((n) => n.type === 'groupNode')) {
        const children = nodes.filter((n) => n.parentId === groupNode.id && n.type !== 'groupNode')
...
export function buildOccupancyMap(
    ...
    for (const node of nodes) {
        ...
        const cell = positionToCell(node.position.x, node.position.y, rows, cols, cellWidth, cellHeight)
        if (cell) {
...
export function computeMinZoom(containerW: number, containerH: number): number {
    ...
    return Math.max(1 - (2 * MARGIN) / containerW, 1 - (2 * MARGIN) / containerH)
}
