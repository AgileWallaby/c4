...
// The -playwright variant bundles Chromium + Playwright deps needed for native PNG/SVG export.
const STRUCTURIZR_PLAYWRIGHT_IMAGE = 'structurizr/structurizr:2026.05.16-playwright'
...
export function createStructurizrPlaywrightContainer(tmpDir: string, logs: string[]): GenericContainer {
    return new GenericContainer(STRUCTURIZR_PLAYWRIGHT_IMAGE)
        .withBindMounts([{ source: tmpDir, target: '/workspace', mode: 'rw' }])
        .withWaitStrategy(Wait.forOneShotStartup())
        .withLogConsumer((stream) => stream.on('data', (chunk) => logs.push(chunk.toString())))
}
